...
const upd = db.prepare(`update requests set in_tok = :in_tok, out_tok = :out_tok, cache_read = :cache_read, cache_create = :cache_create,
  ...
  api_block_index = :api_block_index, model_from_transcript = :model where request_id = :rid`);
...
function line(l: string, path: string) {
  if (!l.includes('"requestId"') || !l.includes('"assistant"')) return; // cheap prefilter; most lines are user/tool rows
  let r: any;
  ...
  const p = {
    ...
    jsonl_path: path, session_id: r.sessionId ?? null, api_block_index: r.apiBlockIndex ?? null, model: r.message.model ?? null,
  };
  const ts = Date.parse(r.timestamp) || Date.now(), s = { rid, cwd: r.cwd ?? null, ts };
